...
fn transfer_checked_with_hook_ix(
    ...
    Instruction {
        ...
        accounts: vec![
            ...
            AccountMeta::new_readonly(*wallet_switch_pda, false),
...
fn test_transfer_switch() {
    ...
    let source_ata = get_associated_token_address(&sender.pubkey(), &mint_keypair.pubkey());
    ...
    let create_source =
        create_associated_token_account_ix(&payer.pubkey(), &sender.pubkey(), &mint_keypair.pubkey());
    let create_dest =
        create_associated_token_account_ix(&payer.pubkey(), &recipient.pubkey(), &mint_keypair.pubkey());
    ...
    send_tx(&mut svm, &[configure_admin_ix], &payer, &[]);
    svm.expire_blockhash();
    ...
    send_tx(&mut svm, &[switch_off_ix], &payer, &[]);
    svm.expire_blockhash();
    ...
    let result = try_send_tx(&mut svm, &[transfer_ix], &sender, &[]);
    assert!(
        result.is_err(),
    ...
    send_tx(&mut svm, &[switch_on_ix], &payer, &[]);
    svm.expire_blockhash();
    ...
    send_tx(&mut svm, &[transfer_ix], &sender, &[]);
}
