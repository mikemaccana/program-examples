...
/// Create a mint with TransferHook extension via raw Token-2022 instructions.
/// 1. CreateAccount with enough space for TransferHook extension
/// 2. InitializeTransferHook (instruction 36)
/// 3. InitializeMint (instruction 0)
fn create_mint_with_transfer_hook_instructions(
    ...
    let token_2022 = token_2022_program_id();
    // Mint with TransferHook extension needs extra space
    // Base mint = 82 bytes, extension overhead = 4 + 2 + 2 + header per extension
    // TransferHook extension: type(2) + length(2) + authority(32) + program_id(32) = 68
    // Account type(1) + extension header = 82 + 1 + 2 + 68 = 153, round up generously
    let mint_space: u64 = 234; // generous to cover TLV headers
    let lamports: u64 = 5_000_000; // enough for rent exemption

    let create_account_ix = anchor_lang::solana_program::system_instruction::create_account(
        payer, mint, lamports, mint_space, &token_2022,
    );

    // InitializeTransferHook: TransferHookExtension(36) + Initialize sub-instruction(0)
    // Data: [36, 0, authority(32), hook_program_id(32)]
    ...
    // InitializeMint (instruction 0)
    // Data: [0, decimals, mint_authority(32), freeze_authority_option(1), freeze_authority(32)]
...
/// TransferChecked instruction for Token-2022 with transfer hook accounts appended.
/// For a transfer hook with extra accounts (counter PDA), the appended accounts are:
///   - ExtraAccountMetaList PDA (readonly)
///   - Counter PDA (writable, as defined in extra account metas)
...
fn test_transfer_hook_counter() {
    ...
    // Step 1: Create mint with TransferHook extension via raw Token-2022 instructions
