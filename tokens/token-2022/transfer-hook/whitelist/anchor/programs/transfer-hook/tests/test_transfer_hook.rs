...
fn transfer_checked_with_hook_ix(
    ...
    Instruction {
        ...
        accounts: vec![
            ...
            AccountMeta::new(*white_list_pda, false),
...
fn test_whitelist_transfer_hook() {
    ...
    let create_source =
        create_associated_token_account_ix(&payer.pubkey(), &payer.pubkey(), &mint_keypair.pubkey());
    ...
    send_tx(&mut svm, &[add_to_whitelist_ix], &payer, &[]);
    svm.expire_blockhash();
